...
- `scripts/context_pack/*.cjs` (11 files) — Node context-pack commands. Report as family. Must stay in parity with `cli/src/context_pack.rs`.
...
- Do not assume a change to `cli/src/context_pack.rs` is complete without also checking `scripts/context_pack/init.cjs` and `seal.cjs`.
